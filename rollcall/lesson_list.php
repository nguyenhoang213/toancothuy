...
$sql = "SELECT bh.MaBuoiHoc, MaLop, Ngay, TenBai FROM buoihoc bh WHERE bh.MaLop = $maLop";
...
<head>
    ...
    <title>Quản lý điểm danh <?php echo htmlspecialchars($class_info['TenLop']); ?></title>
...
<body>
    <div class="content">
        <h1 style="padding-top: 30px">Quản lý điểm danh <?php echo htmlspecialchars($class_info['TenLop']); ?></h1>
        <!-- Nút thêm buổi học mới -->
        <a href="../rollcall/add_lesson.php?MaLop=<?php echo $maLop; ?>" class="add_button">Thêm buổi học mới</a> <br>
        ...
        <table class="lesson_list" style="width: 100%; margin-top: 10px">
            ...
            if ($result->num_rows > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<td><a href='../rollcall/alter_lesson.php?MaBuoiHoc=" . $row['MaBuoiHoc'] . "&MaLop=" . $maLop . "'>Chỉnh sửa</a></td>";
            ...
            <script>
                function redirectToScore(maBuoiHoc, maLop) {
                    window.location.href = `../rollcall/rollcall.php?MaBuoiHoc=${maBuoiHoc}&MaLop=${maLop}`;
                }
            </script>
        ...
        function confirmDelete(maBuoiHoc) {
            if (confirm("Bạn muốn xóa buổi học khỏi hệ thống?")) {
                ...
                if (adminPassword !== null) {
                    fetch('../rollcall/delete_lesson.php', {
                        method: 'POST',
